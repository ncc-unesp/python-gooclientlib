from exceptions import SerializerNotAvailable, SerializerNoAvailable
...
class Serializer(object):
    ...
    def __init__(self, default=None, serializers=None):
        ...
        if not serializers:
            raise SerializerNoAvailable("There are no Available Serializers.")
    ...
    def get_serializer(self, name=None, content_type=None):
        if name is None and content_type is None:
            ...
            if not name in self.serializers:
                raise SerializerNotAvailable("%s is not an available serializer" % name)
            ...
            for x in self.serializers.values():
                for ctype in x.content_types:
                    if content_type == ctype:
                        return x
            raise SerializerNotAvailable("%s is not an available serializer" % content_type)
